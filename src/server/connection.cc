...
BufferedPacket makePacket(Address &address, SharedBuffer<u8> &data,
        u32 protocol_id, u16 sender_peer_id, u8 channel) {
    return makePacket(address, *data, data.getSize(), protocol_id,
            sender_peer_id, channel);
}

SharedBuffer<u8> makeOriginalPacket(SharedBuffer<u8> data) {
    u32 header_size = 1;
...
core::list<SharedBuffer<u8> > makeSplitPacket(SharedBuffer<u8> data,
        u32 chunksize_max, u16 seqnum) {
...
core::list<SharedBuffer<u8> > makeAutoSplitPacket(SharedBuffer<u8> data,
        u32 chunksize_max, u16 &split_seqnum) {
...
RPBSearchResult ReliablePacketBuffer::findPacket(u16 seqnum) {
    ...
    for (; i != m_list.end(); i++) {
        u16 s = readU16(&(i->data[BASE_HEADER_SIZE + 1]));
        /*std::cout<<"findPacket(): finding seqnum="<<seqnum
...
void IncomingSplitBuffer::insert(BufferedPacket &p, bool reliable) {
    ...
    if (chunk_count != sp->chunk_count)
        std::cout << "WARNING: chunk_count=" << chunk_count
                << " != sp->chunk_count=" << sp->chunk_count << std::endl;
    if (reliable != sp->reliable)
        std::cout << "WARNING: reliable=" << reliable << " != sp->reliable="
                << sp->reliable << std::endl;
...
void IncomingSplitBuffer::removeUnreliableTimedOuts(float dtime,
        float timeout) {
    ...
    for (; j != remove_queue.end(); j++) {
        std::cout << "NOTE: Removing timed out unreliable split packet"
                << std::endl;
        delete m_buf[*j];
        m_buf.remove(*j);
    }
...
Connection::Connection(u32 protocol_id, u32 max_packet_size) {
...
void Connection::ServeLogin(unsigned short port) {
    m_socket.Bind(port);
    m_peer_id = PEER_ID_LOGIN_SERVER;
}
...
SharedBuffer<u8> Channel::ProcessPacket(SharedBuffer<u8> packetdata,
        Connection *con, u16 peer_id, u8 channelnum, bool reliable) {
    ...
    if (type == TYPE_CONTROL) {
        ...
        if (controltype == CONTROLTYPE_ACK) {
            ...
            con->PrintInfo();
            std::cout << "Got CONTROLTYPE_ACK: channelnum="
            ...
            try {
                ...
                peer->avg_rtt = rtt * 0.1 + peer->avg_rtt * 0.9;
                // Calculate resend_timeout
                float resend_timeout = peer->avg_rtt * 2.0;
                if (resend_timeout < RESEND_TIMEOUT_MIN)
                    resend_timeout = RESEND_TIMEOUT_MIN;
                if (resend_timeout > RESEND_TIMEOUT_MAX)
                    resend_timeout = RESEND_TIMEOUT_MAX;
                peer->resend_timeout = resend_timeout;
                ...
                con->PrintInfo();
                std::cout << "WARNING: ACKed packet not in outgoing queue"
                        << std::endl;
            }
            ...
            con->PrintInfo();
            std::cout << "Got new peer id: " << peer_id_new << "... ";

            if (con->GetPeerID() != PEER_ID_NEW) {
                std::cout << "WARNING: not changing." << std::endl;
            } else {
            ...
            con->PrintInfo();
            std::cout << "INVALID TYPE_CONTROL: invalid controltype="
        ...
        con->PrintInfo();
        std::cout << "RETURNING TYPE_ORIGINAL to user" << std::endl;
        ...
        BufferedPacket packet = makePacket(con->GetPeer(peer_id)->address,
                packetdata, con->GetProtocolID(), peer_id, channelnum);
        try {
            // Buffer the packet
        ...
        // are collected.
            ...
            con->PrintInfo();
            std::cout << "RETURNING TYPE_SPLIT: Constructed full data, "
                    << "size=" << e.getData().getSize() << std::endl;
            return e.getData();
        }
        con->PrintInfo();
        std::cout << "BUFFERING TYPE_SPLIT" << std::endl;
        ...
        if (is_future_packet)
            ...
            std::cout << "RECUR";
        std::cout << " TYPE_RELIABLE seqnum=" << seqnum << " next="
                << next_incoming_seqnum;
        std::cout << " [sending CONTROLTYPE_ACK"
                " to peer_id=" << peer_id << "]";
        std::cout << std::endl;
        ...
        if (is_future_packet) {
            /*con->PrintInfo();
             std::cout<<"Buffering reliable packet (seqnum="
             <<seqnum<<")"<<std::endl;*/
        ...
        con->PrintInfo();
        std::cout << "Got invalid type=" << ((int) type & 0xff) << std::endl;
...
SharedBuffer<u8> Channel::CheckIncomingBuffers(Connection *con, u16 &peer_id) {
    u16 firstseqnum = 0;
    ...
    if (incoming_reliables.empty() == false) {
        if (firstseqnum == next_incoming_seqnum) {
            ...
            con->PrintInfo();
            std::cout << "UNBUFFERING TYPE_RELIABLE" << " seqnum=" << seqnum
                    << " peer_id=" << peer_id << " channel="
                    << ((int) channelnum & 0xff) << std::endl;
...
u32 Connection::Receive(u16 &peer_id, u8 *data, u32 datasize) {
    ...
    for (;;) {
        try {
            ...
            // receive the data by querying the socket
            s32 received_size = m_socket.Receive(sender, *packetdata,
                    packet_maxsize);
            ...
            if (channelnum > CHANNEL_COUNT - 1) {
                PrintInfo();
                std::cout << "Receive(): Invalid channel " << channelnum
                        << std::endl;
            ...
            if (peer_id == PEER_ID_NEW) {
                ...
                // Else: A peer was found.
                    ...
                    peer_id = peer->id;
                    PrintInfo();
                    std::cout << "WARNING: Assuming unknown peer to be "
            ...
            if (peer_id == PEER_ID_NEW) {
                ...
                for (; j.atEnd() == false; j++) {
                    Peer *peer = j.getNode()->getValue();
                    if (peer_id_new <= peer->id)
                        peer_id_new = peer->id + 1;
                }

                PrintInfo();
                std::cout << "Receive(): Got a packet with peer_id=PEER_ID_NEW,"
            ...
            // add new peer ID, for the peer ID specified by the client
            if (!m_peers.find(peer_id)) {
                // Create a peer
                Peer *peer = new Peer();
                peer->address = sender;
                peer->id = peer_id;
            ...
            if (node == NULL) {
                // Peer not found
                PrintInfo();
                std::cout << "Receive(): Peer not found" << std::endl;
                throw InvalidIncomingDataException(
                        "Peer not found (possible timeout)");
            }

            Peer *peer = node->getValue();

            //TODO: Validate peer address
            ...
            try {
                ...
                PrintInfo();
                std::cout << "ProcessPacket returned data of size "
...
void Connection::SendToAll(u8 channelnum, SharedBuffer<u8> data,
        bool reliable) {
    core::map<u16, Peer*>::Iterator j;
    j = m_peers.getIterator();
    for (; j.atEnd() == false; j++) {
...
void Connection::Send(u16 peer_id, u8 channelnum, SharedBuffer<u8> data,
        bool reliable) {
...
void Connection::SendAsPacket(u16 peer_id, u8 channelnum, SharedBuffer<u8> data,
        bool reliable) {
    Peer *peer = GetPeer(peer_id);
    Channel *channel = &(peer->channels[channelnum]);
    ...
    if (reliable) {
        ...
        BufferedPacket p = makePacket(peer->address, reliable, m_protocol_id,
                m_peer_id, channelnum);
        ...
        try {
            ...
            PrintInfo();
            std::cout << "WARNING: Going to send a reliable packet "
        ...
        // Add base headers and make a packet
        BufferedPacket p = makePacket(peer->address, data, m_protocol_id,
                m_peer_id, channelnum);
...
void Connection::RunTimeouts(float dtime) {
    ...
    for (; j.atEnd() == false; j++) {
        ...
        // TODO: Get the constant timeout from somewhere else
        if (peer->timeout_counter > 30.0) {
            PrintInfo();
            std::cout << "RunTimeouts(): Peer " << peer->id << " has timeouted."
                    << std::endl;
        ...
        for (u16 i = 0; i < CHANNEL_COUNT; i++) {
            ...
            core::list<BufferedPacket> timed_outs =
                    channel->outgoing_reliables.getTimedOuts(resend_timeout);

            channel->outgoing_reliables.resetTimedOuts(resend_timeout);

            core::list<BufferedPacket>::Iterator j = timed_outs.begin();
            for (; j != timed_outs.end(); j++) {
                ...
                PrintInfo();
                std::cout << "RE-SENDING timed-out RELIABLE: " << "peer_id="
                        << peer_id << ", channel=" << ((int) channel & 0xff)
                        << ", seqnum=" << seqnum << std::endl;
                RawSend(*j);
            }
            ...
            channel->incoming_splits.removeUnreliableTimedOuts(dtime, 5.0);
        }
    }
    ...
    for (; i != timeouted_peers.end(); i++) {
        PrintInfo();
        std::cout << "RunTimeouts(): Removing peer " << (*i) << std::endl;
        m_peers.remove(*i);
    }
}
...
Peer* Connection::GetPeer(u16 peer_id) {
    ...
    if (node == NULL) {
        // Peer not found
        throw NotFoundException("Peer not found (possible timeout)");
    }
...
void Connection::PrintInfo() {
    /*for(u16 i=0; i<m_indentation; i++)
     std::cout<<"  ";
     std::cout<<"Connection(h="<<m_socket.GetHandle()<<", id="
     <<m_peer_id<<"): ";*/
    std::cout << m_socket.GetHandle();
    std::cout << " ";
    std::cout << "con " << m_peer_id << ": ";
