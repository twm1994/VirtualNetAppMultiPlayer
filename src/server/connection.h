...
irr::core::list<SharedBuffer<u8> > makeSplitPacket(SharedBuffer<u8> data,
        u32 chunksize_max, u16 seqnum);

// Depending on size, make a TYPE_ORIGINAL or TYPE_SPLIT packet
// Increments split_seqnum if a split packet is made
irr::core::list<SharedBuffer<u8> > makeAutoSplitPacket(SharedBuffer<u8> data,
...
struct IncomingSplitPacket {
    ...
    irr::core::map<u16, SharedBuffer<u8> > chunks;
    u32 chunk_count;
...
#define PEER_ID_LOGIN_SERVER 99
...
typedef irr::core::list<BufferedPacket>::Iterator RPBSearchResult;
...
public:
    ...
    irr::core::list<BufferedPacket> getTimedOuts(float timeout);

private:
    irr::core::list<BufferedPacket> m_list;
};
...
private:
    // Key is seqnum
    irr::core::map<u16, IncomingSplitPacket*> m_buf;
};
...
struct Channel {
     ...
     */
    SharedBuffer<u8> ProcessPacket(SharedBuffer<u8> packetdata, Connection *con,
            u16 peer_id, u8 channelnum, bool reliable = false);

    // Returns next data from a buffer if possible
    // throws a NoIncomingDataException if no data is available
    // If found, sets peer_id
    SharedBuffer<u8> CheckIncomingBuffers(Connection *con, u16 &peer_id);
...
public:
    ...
    void setTimeoutMs(int timeout) {
        m_socket.setTimeoutMs(timeout);
    }
    // Start being a server
    void Serve(unsigned short port);

    void ServeLogin(unsigned short port);
